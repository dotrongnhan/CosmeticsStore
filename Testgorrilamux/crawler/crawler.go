...
func CrawlProduct(db *sql.DB)  {
	...
	//send request get HTML
	c.OnRequest(func(r *colly.Request) {
		fmt.Printf("Visiting: %sn", r.URL)
	})

	//handle error when crawl html
	...
	c.OnHTML(".productGrid", func(e *colly.HTMLElement) {
		product := models.Product{}
		product.ProductName = e.ChildText(".yQmmFK")
		priceTypeText := e.ChildText("._5W0f35>._29R_un")
		price := strings.Replace(priceTypeText, ".", "", -1)
		product.Price, _ = strconv.ParseFloat(price, 64)
		product.Image = e.ChildAttr(".mxM4vG", "src")
		...
		fmt.Printf("- Name: %s- Price: %f- Picture: %s\n", product.ProductName, product.Price, product.Image) //In ra màn hình giá trị đã lấy được
	...
	c.Visit("https://shopee.vn/search?facet=100892&keyword=skii&noCorrection=true&page=0")
}